<template>
  <div v-if="show" class="modal-overlay" @click.self="closeModal">
    <div class="modal-container">
      ...
      <textarea
        v-model="content"
        :placeholder="t('addReviewModal.placeholder')"
      ...
      <div class="flex flex-col items-center mx-auto gap-2">
        <label
          class="text-black border border-black text-center w-full py-1 rounded-full cursor-pointer"
        >
          {{ t('addReviewModal.uploadPhoto') }}
          <input type="file" @change="handleFileUpload" hidden />
        </label>
        <button
          @click="submitReview"
          class="border bg-black text-white w-full text-center py-1 rounded-full cursor-pointer"
        >
